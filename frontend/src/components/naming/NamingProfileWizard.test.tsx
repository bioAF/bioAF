...
describe("number segments", () => {
  ...
  test("user-added number segments do not show a padding input either", async () => {
    // Padding is a write-time concept and the parser is lenient at read
    // time (spec section 8.7); since this feature is parse-only the input
    // had no observable effect, so it's not surfaced anywhere.
    const user = userEvent.setup();
    renderWizard();
