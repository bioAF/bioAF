...
- ``PUT /api/beta-features/{key}`` toggles a flag. Admin-only (``infrastructure:configure``); an
  unknown key is 404. There is no instance-level gate: the ``@bioaf.co`` availability check was
  removed, because it made every beta feature internal-only on any instance bioAF does not staff.
"""
