...
_SYSTEM = (
    ...
    "- Record every gap you filled in `assumptions`, in your own words. 'The paper does not name a "
    "normalization, so DESeq2's median-of-ratios was used' is exactly the kind of thing a disputing "
    "reader needs, and an unstated assumption is worse than a stated one.\n"
    "- Never assume a threshold. Apply no significance or fold-change cutoff in the script: bioAF "
    "applies the cutoff the paper states, exactly as stated, to the table you write.\n"
    "- Write the results to a CSV with an id column, a log2 fold change, a raw p-value and an adjusted "
    "p-value, one row for every feature tested and none filtered out, so the paper's own definition "
