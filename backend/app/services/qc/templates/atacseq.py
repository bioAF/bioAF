...
def read_multiqc_metrics(multiqc_json_text: str) -> dict[str, Any]:
    ...
    # MACS2 peak count + FRiP (nf-core/atacseq custom content; names vary -> defensive + general-stats fallback)
...
def generate_summary(metrics: dict[str, Any]) -> str:
    ...
    if n and total is not None:
        summary = f"This ATAC-seq run analyzed **{n} samples** with a mean of **{total:,} reads per sample**."
    elif n:
        summary = f"This ATAC-seq run analyzed **{n} samples**."
