...
def parse_sheet_url(url: str) -> tuple[str, int | None]:
    ...
    Raises ``ValidationError`` for unrecognised formats.
    """
    m = _SHEET_URL_RE.search(url)
    if not m:
        raise ValidationError("Not a valid Google Sheets URL")
...
def _resolve_sheet_name(
    ...
    if not sheets:
        raise ValidationError("Spreadsheet has no sheets")
    if gid is not None:
        for s in sheets:
            if s["properties"].get("sheetId") == gid:
                return s["properties"]["title"]
        raise ValidationError(f"No sheet found with gid={gid}")
...
def read_header_row(
    ...
    if not values or not values[0]:
        raise ValidationError("The first row is empty -- ensure row 1 contains column headers")
...
def read_all_rows_as_csv(
    ...
    if not rows:
        raise ValidationError("The sheet is empty")
