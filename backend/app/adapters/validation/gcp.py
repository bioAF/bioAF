"""GCP credentials validation provider (Stage 3b).

The GCP-specific account/credentials validation, relocated under ``adapters/``
so its google-cloud SDK clients (``resourcemanager_v3``, ``storage``,
``service_usage_v1``, ``container_v1``) live behind the BAL boundary instead of
in the service layer. ``app.services.gcp_config`` is now a thin re-export shim.
The validate endpoint is itself cloud-specific (an AWS install validates a
different way), so this is a relocation rather than a runtime backend seam; an
AWS validation provider slots in here as a sibling module later.
...
def _load_manifest() -> dict:
    ...
    # backend/app/adapters/validation/gcp.py -> repo root via parents[4]
    candidate = here.parents[4] / "installer" / "roles_manifest.yaml"
