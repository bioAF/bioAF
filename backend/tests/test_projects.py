...
async def test_list_projects_counts_experiments_via_project_id(client, admin_token, session, admin_user):
    ...
    s1 = Sample(experiment_id=exp1.id, external_id="CNT-1", organism="Homo sapiens", status="registered")
    s2 = Sample(experiment_id=exp1.id, external_id="CNT-2", organism="Homo sapiens", status="registered")
    s3 = Sample(experiment_id=exp2.id, external_id="CNT-3", organism="Homo sapiens", status="registered")
...
async def test_project_detail_counts_experiments_via_project_id(client, admin_token, session, admin_user):
    ...
    s1 = Sample(experiment_id=exp.id, external_id="DET-1", organism="Homo sapiens", status="registered")
    session.add(s1)
    await session.commit()
