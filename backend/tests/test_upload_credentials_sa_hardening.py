...
async def test_get_gcs_credentials_returns_impersonated_creds_in_vm_default(session):
    ...
    with patch("app.adapters.credentials.credential_injector.load_gcp_credentials", return_value=fake_creds) as load:
...
async def test_storage_service_query_gcs_buckets_uses_impersonated_creds(session):
    ...
    with (
        patch("app.adapters.credentials.credential_injector.load_gcp_credentials", return_value=fake_creds) as load,
