...
# ---------------------------------------------------------------------------
# bioaf-app (runtime data-plane SA) email, read from the VM's attached SA
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_persist_app_sa_skipped_when_metadata_unreachable(session):
    """No bioaf_app_sa_email row written when the metadata server is unreachable."""
    with patch(
        "app.services.bootstrap_metadata.get_attached_sa_email",
        new_callable=AsyncMock,
        return_value=None,
    ):
        ok = await persist_app_sa_from_metadata(session)
    assert ok is False
    row = (await session.execute(text("SELECT value FROM platform_config WHERE key='bioaf_app_sa_email'"))).scalar()
    assert row is None


@pytest.mark.asyncio
async def test_persist_app_sa_writes_value_from_metadata(session):
    """Reads the VM's attached SA and upserts platform_config."""
    with patch(
        "app.services.bootstrap_metadata.get_attached_sa_email",
        new_callable=AsyncMock,
        return_value="bioaf-app@my-project.iam.gserviceaccount.com",
    ):
        ok = await persist_app_sa_from_metadata(session)
    assert ok is True
    row = (await session.execute(text("SELECT value FROM platform_config WHERE key='bioaf_app_sa_email'"))).scalar()
    assert row == "bioaf-app@my-project.iam.gserviceaccount.com"


@pytest.mark.asyncio
async def test_persist_app_sa_does_not_overwrite_existing_value(session):
    """If the row already exists, leave it untouched (admin override safe)."""
    await session.execute(
        text(
            "INSERT INTO platform_config (key, value) VALUES "
            "('bioaf_app_sa_email', 'manual-override@my-project.iam.gserviceaccount.com')"
        )
    )
    await session.commit()
