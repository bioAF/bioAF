...
async def test_sample_batch_code_creates_batch(client, admin_token, session):
    ...
    resp = await client.post(
        ...
        json={"external_id": "S001", "sample_batch_code": "prep-42"},
...
async def test_sample_batch_code_reuses_existing(client, admin_token, session):
    ...
    await client.post(
        f"/api/experiments/{exp_id}/samples",
        json={"external_id": "S002", "sample_batch_code": "prep-42"},
        headers={"Authorization": f"Bearer {admin_token}"},
    )
    await client.post(
        f"/api/experiments/{exp_id}/samples",
        json={"external_id": "S003", "sample_batch_code": "prep-42"},
        headers={"Authorization": f"Bearer {admin_token}"},
    )
...
async def test_sequencing_batch_code_creates_batch(client, admin_token, session):
    ...
    resp = await client.post(
        f"/api/experiments/{exp_id}/samples",
        json={"external_id": "S004", "sequencing_batch_code": "cro123"},
        headers={"Authorization": f"Bearer {admin_token}"},
    )
    assert resp.status_code == 200
    data = resp.json()
...
async def test_sequencing_batch_code_shared_across_experiments(client, admin_token, session):
    ...
    await client.post(
        ...
        json={"external_id": "S005", "sequencing_batch_code": "cro-shared"},
        headers={"Authorization": f"Bearer {admin_token}"},
    )
    await client.post(
        f"/api/experiments/{exp2}/samples",
        json={"external_id": "S006", "sequencing_batch_code": "cro-shared"},
        headers={"Authorization": f"Bearer {admin_token}"},
    )
