...
DECLARATION = [
    ...
    {"name": "marker_panel", "type": "string", "required": False, "binding": {"source": "custom_field", "key": "panel"}},
...
async def world(session):
    ...
    exp = Experiment(
        name="Imaging", organization_id=org.id, status="fastq_uploaded", owner_user_id=admin.id
    )
    session.add(exp)
    await session.flush()
...
class TestADeclarationReachesTheFile:
    ...
    async def test_the_declared_columns_are_the_sheet(self, client, world):
        ...
        r = await _preflight(
            client, headers, world, sample_values={str(world["sample"].id): {"cycle": "1"}}
        )
