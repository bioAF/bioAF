...
def _row(column, arm, unit, *, group=None, time="day 0"):
    return {
        ...
        "biological_sample": f"{unit} {arm} {time}",
        "technical_group": group,
        "time_point": time,
        "evidence": [{"source": "sample_record", "quote": "x"}],
    }
...
def test_two_donors_under_treatment_and_control_with_technical_duplicates_keep_their_pairs():
    ...
    mapping = [
        _row("D1_T_a", "test", "D1", group="D1T"),
        _row("D1_T_b", "test", "D1", group="D1T"),
        _row("D2_T_a", "test", "D2", group="D2T"),
        _row("D2_T_b", "test", "D2", group="D2T"),
        _row("D1_C_a", "reference", "D1", group="D1C"),
        _row("D1_C_b", "reference", "D1", group="D1C"),
        _row("D2_C_a", "reference", "D2", group="D2C"),
        _row("D2_C_b", "reference", "D2", group="D2C"),
    ]
...
def test_a_technical_group_spanning_two_conditions_is_rejected_and_the_design_holds():
    mapping = [
        _row("a", "test", "D1", group="g"),
        _row("b", "reference", "D1", group="g"),
        _row("c", "test", "D2"),
        _row("d", "reference", "D2"),
    ]
...
def test_several_biological_samples_of_one_unit_in_one_arm_are_not_collapsed_and_hold():
    mapping = [
        _row("a", "test", "clone 5"),
        {**_row("b", "test", "clone 5"), "biological_sample": "clone 5 culture 2"},
        _row("c", "reference", "WT1"),
        _row("d", "reference", "WT2"),
    ]
    _, status, reason = design_from_mapping(_DESIGN, mapping, contrast_index=0)
    assert status == "unsupported"
...
def test_a_paired_design_whose_names_change_during_mapping_keeps_its_pairing():
    ...
    mapping = [
        _row("A_treated", "test", "donorA"),
        _row("B_treated", "test", "donorB"),
        _row("A_control", "reference", "donorA"),
        _row("B_control", "reference", "donorB"),
    ]
...
def test_only_the_selected_contrast_is_rewritten():
    design = {
        "contrasts": [dict(_DESIGN["contrasts"][0]), {"name": "day 7", "test_samples": ["x"], "reference_samples": []}]
    }
    mapping = [
        _row("a", "test", "D1"),
        _row("b", "test", "D2"),
...
def test_a_pick_matching_several_columns_is_a_duplicate_never_added_under_one_pairing_label():
    ...
    associations = [
        {"column": "s1a", "accessions": ["GSM1"]},
        {"column": "s1b", "accessions": ["GSM1"]},
        {"column": "s2", "accessions": ["GSM2"]},
        {"column": "s3", "accessions": ["GSM3"]},
...
@pytest.mark.parametrize(
    "name,rule",
    [
        ("de_bulk_deseq2.ipynb", "rowSums"),
        ("da_peaks_deseq2.ipynb", "rowSums"),
...
def test_the_deposit_templates_collapse_by_technical_group_alone(name, rule):
    ...
    assert source.rindex("technical_group_labels") < source.index(
        "DESeqDataSetFromMatrix(" if "deseq2" in name else "lmFit("
    )
...
@pytest.mark.parametrize(
    "name", ["de_bulk_deseq2.ipynb", "da_peaks_deseq2.ipynb", "de_normalized_limma.ipynb", "de_pseudobulk_deseq2.ipynb"]
)
